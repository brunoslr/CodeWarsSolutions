/* URL https://www.codewars.com/kata/simple-fun-number-87-shuffled-array/train/csharp
A noob programmer was given two simple tarefas: sum and sort the elements of the given array arr = [a1, a2, ..., an].
...
{
    public class ShuffledArrayKata
    {
        public int[] ShuffledArray(int[] shuffled)
        {
            return shuffled
            .Where(x => x != shuffled.Sum() / 2)
            .Concat(shuffled.Where(x => x == shuffled.Sum() / 2).Skip(1))
            .OrderBy(x => x).ToArray();
