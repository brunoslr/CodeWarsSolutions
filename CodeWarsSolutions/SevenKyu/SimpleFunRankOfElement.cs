/* URL: https://www.codewars.com/kata/simple-fun-number-177-rank-of-element/train/csharp
...
{
    public class SimpleFunRankOfElement
    {
        public int RankOfElement(int[] arr, int i)
        {

            return arr.Where((a, b) => a <= arr[i] && b < i).Count()
                + arr.Where((a, b) => a < arr[i] && b > i).Count();
